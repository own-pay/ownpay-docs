...
The official OwnPay Node.js SDK is a zero-dependency, TypeScript-first package for integrating payments into your server-side. It uses native `fetch` and `crypto` modules with built-in retries and webhook signature verification.

<CardGroup cols={2}>
  <Card title="npm" icon="box" href="https://www.npmjs.com/package/ownpay-nodejs">
    Install via npm, yarn, or pnpm.
  </Card>

  <Card title="GitHub" icon="brand-github" href="https://github.com/own-pay/ownpay-nodejs">
...
<Tabs>
  <Tab title="ESM">
    ...
    const client = new OwnPay('op_live_abc123.def456');
    
    // With configuration options
    const client = new OwnPay({
      apiKey: 'op_live_abc123.def456',
...
| `currency` | `string` | Yes | 3-letter ISO currency code (for example, `BDT`, `USD`) |
...
interface WebhookPayload {
  event: string;              // for example, 'payment.completed'
