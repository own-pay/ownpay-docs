...
| Environment config | `.env` | APP_KEY, ENCRYPTION_KEY, JWT_SECRET, database credentials, Redis config |
...
```bash
gunzip < /backups/ownpay/db-2025-01-15_020000.sql.gz | mysql -u ownpay_user -p ownpay_db
```

### Restore application files

```bash
tar -xzf /backups/ownpay/files-2025-01-15_020000.tar.gz -C /var/www/
cp /backups/ownpay-env-2025-01-15.txt /var/www/ownpay/.env
chown -R www-data:www-data /var/www/ownpay
chmod 600 /var/www/ownpay/.env
```
...
rm -rf /var/www/ownpay/storage/cache/*
rm -rf /var/www/ownpay/storage/sessions/*

# Restart services
sudo systemctl start php8.3-fpm nginx
```

Visit the admin panel and verify that brands, transactions, and settings are intact. Check the health endpoint: `curl https://your-domain.com/api/v1/health`.
