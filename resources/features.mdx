...
- **Invisible infrastructure** - when a customer reaches checkout, they land on `pay.yourbrand.com`. They see the brand's logo, name, and colors. OwnPay is not mentioned at, no shared domain, no "powered by" indicator unless the brand explicitly adds one.
...
| Custom slug | Memorable URL path (for example, `/pay/annual-subscription`) |
...
| Filters (`applyFilter`) | Transform values in-flight (for example, modify payment amount, alter checkout output) |
