...
> Looking to run it locally first? See [**LOCAL\_SETUP.md**](/resources/local-setup).
...
The architecture uses **no framework** (no Laravel/Symfony runtime). The kernel, router, container, and middleware pipeline are small, readable, first-party code under `src/`.
...
3. Set the timezone.
4. **Boot plugins** (`PluginLoader::boot()`) - _before_ middleware, so a plugin can inject middleware via the `system.middleware.pipeline` filter.
...
### 4.5 White-label custom domains _(industry first)_

OwnPay is the **first self-hosted payment platform** to support per-brand custom-domain checkout on a single installation. A brand maps its own domain (for example `pay.yourbrand.com`), which is DNS-verified (TXT ownership \+ A-record routing), then fully activated. From that point, every customer-facing URL uses the brand's domain - customers see the brand's logo, name, and colors; there is no trace of OwnPay.
...
Runtime settings live in `op_system_settings` (group/key/value, with an optional `merchant_id` for per-brand overrides). Access them through `EnvironmentService` / `SettingsRepository`; `EnvironmentService::get()` falls back to OS environment variables when no stored value exists. The system includes no separate config store.
...
- [Features](/resources/features) - Complete list of OwnPay capabilities
- [Roadmap](/resources/roadmap) - Upcoming releases and milestones
- [Security and Compliance](/resources/security-compliance) - Encryption, PCI, and hardening guide
- [Contributing](/resources/contributing) - How to contribute code and plugins
